...
int getGameSpeed(void)
{
	...
	{
		gotoxy(10,5);
		printf("Dificuldade do Jogo (Insira um num. de 1 a 9): ");
...
void createHighScores(void)
{
	...
	if(file == NULL)
	{
		printf("ERROR: FAILED TO CREATE HIGHSCORES!!! EXITING!");
		exit(0);	
	}
...
void inputScore(int score) //This seriously needs to be cleaned up
{
	...
	gotoxy(10,5);
	printf("Voce pontuou entre os 5 melhores da nossa historia!");
	gotoxy(10,6);
...
//	printf("\t\t    |        ~~~~~~~~~         ~~~~~~~~ 		\n");
//	printf("\t\t    ^											\n");
	gotoxy(2, 2); printf("Bem vindo ao CSnake !!!\n");
	gotoxy(2, 3); printf("Pressione qualquer tecla para continuar...");
//	printf("\n");
...
void controls(void)
{
	...
	printf("Controles\n");
	gotoxy(x++,y++);
	printf("Utilize as seguintes teclas para direcionar o movimento da cobrinha: ");
	gotoxy(x,y++);
...
void exitYN(void)
{
	...
	gotoxy(2, 2);
	printf("Poxa, vida! Você realmente desejas sair (S/N)? ::> ");
	
	do
	{
		pressed = waitForAnyKey();
		pressed = tolower(pressed);
	} while (!(pressed == 's' || pressed == 'n'));
...
int mainMenu(void)
{
	...
	gotoxy(x,y++); printf("	Novo Jogo\n");
	gotoxy(x,y++); printf("	Recordes\n");
	gotoxy(x,y++); printf("	Ajuda\n");
	gotoxy(x,y++); printf("	Sair\n"); 
